...
function sepaHashProcess(value)
{
...
function getSepaFormValues()
{
    ...
    var requestParams = {'sepaAccountHolder':sepaAccountHolder, 'sepaBankCountry':$nnsepa_j('#novalnetSepa_bank_country').val(),
        'sepaAccountNumber':sepaAccountNumber, 'sepaUniqueId':sepaUniqueId,
        'merchantVendor':$nnsepa_j('#process_vendor_id').val(), 'merchantAuthcode':$nnsepa_j('#auth_code').val()};
...
function sepaRefillcall()
{
    ...
    $nnsepa_j('#sepa_loading').show();
    var sepaPayportParams = "vendor_id="+$nnsepa_j('#process_vendor_id').val()+"&vendor_authcode="+$nnsepa_j('#auth_code').val()+"&unique_id="+sepaUniqueId+"&sepa_data_approved=1&mandate_data_req=1&sepa_hash="+sepaPanHash;
...
function sepaOneClickShopping()
{
    if ($nnsepa_j('#nnSepa_oneclick_shopping').val() ==undefined
        ...
        $nncc_j('#sepa_oneclick_link, #sepa_oneclick_given').css('display','none');
        $nncc_j('#sepa_local_form').css('display','block');
    }
}

function oneClickShopConfirm(val)
{
    var SepaConfirm = $nnsepa_j('#novalnetSepa_mandate_confirm_ocs').is(':checked');
    $nnsepa_j('#nnsepa_iban_confirmed').val(0);
    if (SepaConfirm ==true) {
        $nnsepa_j('#nnsepa_iban_confirmed').val(1);
        $nnsepa_j('#nnSepa_new_form').val(0);
